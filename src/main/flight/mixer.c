...
#ifdef USE_DYN_IDLE
        if ((!mixerConfig()->rpm_limiter_linearization || !mixerConfig()->rpm_limiter) && mixerRuntime.dynIdleMinRps > 0.0f) {
...
static void applyRPMLimiterLinearized(void) 
{
    if (mixerConfig()->rpm_limiter && motorConfig()->dev.useDshotTelemetry && ARMING_FLAG(ARMED)) {
        ...
        float rpmLimiterD = (smoothedRPMError - mixerRuntime.rpmLimiterPreviousSmoothedRPMError) * mixerRuntime.rpmLimiterDGain;
        if (throttle < motorConfig()->digitalIdleOffsetValue / 10000.0f) {
            mixerRuntime.rpmLimiterI *= 1.0f/(1.0f+(pidGetDT()*10.0f));
        }
        ...
        // float curveError = throttle - pidOutput;
...
static void applyRPMLimiter(void)
{
    if (mixerConfig()->rpm_limiter && motorConfig()->dev.useDshotTelemetry && ARMING_FLAG(ARMED)) {
        ...
        if (smoothedRPMError > 0.0f && rcCommand[THROTTLE] < rxConfig()->maxcheck) {
            mixerRuntime.rpmLimiterExpectedThrottleLimit *= 1.0f - 4.8f * pidGetDT();
        } else if (pidOutput < -400 * pidGetDT() && rcCommand[THROTTLE] >= rxConfig()->maxcheck && !isMotorSaturated()) { // Throttle accel corresponds with motor accel
            mixerRuntime.rpmLimiterExpectedThrottleLimit *= 1.0f + 3.2f * pidGetDT();
        }
